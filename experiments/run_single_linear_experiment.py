...
DEFAULT_OUTDIR = os.path.join(ROOT_DIR, "experiments", "results", "single_linear")
...
def eval_dynamics_curve(v, w, n, t_list, d, de, batch_size, use_history, device):
...
def main():
    parser = argparse.ArgumentParser(description="Run one fixed (D, M, N) linear experiment.")
    parser.add_argument("--d", type=int, default=50, help="Dimension D.")
    parser.add_argument("--m", type=int, default=100, help="Number of tasks M.")
    ...
    parser.add_argument("--use-history", action="store_true", help="Use history at inference.")
    ...
    print(f"Saved single linear experiment to: {args.outdir}")
