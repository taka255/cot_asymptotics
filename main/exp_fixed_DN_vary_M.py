...
print(
    f"device={device}, D={D}, N={N}, M_LIST={M_LIST}, "
...
def build_fixed_train_dataset(m, n=N):
...
def main():
    ...
    for i, m in enumerate(M_LIST):
        # Keep per-M runs reproducible but independent.
        ...
        results_by_m[m] = final_eval

        print(f"\n=== fixed D={D}, N={N}, M={m} ===")
        for t in EVAL_T_LIST:
            print(f"M={m:>6}, t={t:>2}: {final_eval[t]:.6f}")

    plt.figure(figsize=(7.5, 4.8))
    for m in M_LIST:
        ts = np.array(sorted(results_by_m[m].keys()))
        vals = np.array([results_by_m[m][t] for t in ts])
        plt.plot(ts, vals, marker="o", linewidth=2, label=f"M={m}")

    plt.xlabel("test-time step t")
    plt.ylabel("test loss (0.5 * ||w_hat_t - w*||^2)")
    plt.title(f"Fixed D={D}, N={N}; finite-M training (w0~N(0,I), mask)")
